...
import (
	//"Gobelieve/account_rpc/pb"
	"context"
	"fmt"
	"time"

	pb "gotalk/account_rpc"

	log "github.com/golang/glog"
	"github.com/gomodule/redigo/redis"

	//grpc "github.com/grpc-go-master"
	"google.golang.org/grpc"
)

// account_rpc 地址
const AccountRPCHost = "120.77.66.96:8003"
...
func LoadUserAccessToken(token string) (int64, int64, int, bool, error) {
	...
	var forbidden int
	conn, err := grpc.Dial(AccountRPCHost, grpc.WithInsecure())
	if err != nil {
		log.Fatalf("did not connect:%v", err)
	}
	defer conn.Close()
	c := pb.NewSsoClient(conn)

	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()

	resp, err := c.GetUserInfo(ctx, &pb.SsoRequest{
		Source:      "AQIDAA==",
		SessionName: token,
	})
	log.Info("toekn是", token)
	log.Info("account rpc返回:", resp)
	if err != nil {
		log.Fatalf("could not get uesr info: %v", err)
	}
	if resp.ErrorCode != 10000 {
		log.Info("�û���֤ʧ��")
		//uid ���� 0, ��֤ʧ�ܺ�
